...
async fn spawn_app() -> TestApp {
    ...
    // Get the port defined by the OS
    let port = listener.local_addr().unwrap().port();
    let address = format!("http://127.0.0.1:{}", port);

    let mut configuration = get_configuration().expect("Failed to read configuration");
    configuration.database.database_name = Uuid::new_v4().to_string(); // Generate a random database for each test

    let connection_pool = configure_database(&configuration.database).await;

    let server =
        zero2prod::startup::run(listener, connection_pool.clone()).expect("Failed to bind address");
    let _ = tokio::spawn(server); // let tokio handle the spawning of the server as a background task
    TestApp {
        address,
...
async fn subscribe_returns_a_400_when_data_is_missing() {
    ...
    for (invalid_body, error_message) in test_cases {
        ...
        assert_eq!(
            ...
            // Custom messagem on test failure
