...
async fn new_password_is_too_short() {
    ...
    let html_page = app.get_change_password_html().await;
    assert!(html_page.contains(
        "<p><i>The new password is too short. It should have 8 or more characters.</i></p>"
    ));
}

#[tokio::test]
async fn new_password_is_too_big() {
    // Arrange
    let app = spawn_app().await;
    let current_password = &app.test_user.password;
    let big_password = String::new().add("a").repeat(130);
    ...
    let response = app
        .post_change_password(&serde_json::json!({
            ...
            "new_password_check": &big_password
        }))
        .await;
    ...
    let html_page = app.get_change_password_html().await;
    assert!(html_page.contains(
        "<p><i>The new password is too long. It should have 129 or fewer characters.</i></p>"
    ));
}
